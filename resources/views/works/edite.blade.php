            ...
            <div class="card-body pt-0">
                <form class="form-horizontal" method="POST" action="{{ route('work.update' , $work->id) }}"
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-md-10">
                            <textarea id="site_title" name="title_ar"
                                class="form-control">{{ $work->title_ar }}</textarea>
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-md-10">
                            <textarea id="site_title" name="title_en"
                                class="form-control">{{ $work->title_en }}</textarea>
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="col-md-2 control-label">Category:</label>
                        <div class="col-md-10">
                            <select name="id_category" id="select-beast"
                                class="form-control  nice-select  custom-select">
                                @foreach($category as $category)
                                <option value="{{ $category->id }}" @if($category->id == $work->id_category)) selected
                    ...
                    <div class="form-group">
                        ...
                        <div id="delete-{{ $attachments->id }}" class="d-inline">
                            <button id="delete-{{ $attachments->id }}" type="submit"
                                class="modal-effect btn btn-sm btn-danger"><i class="las la-trash"></i></button>
                            <div class="card-aside-img d-inline">
                                <img src="{{ asset('uploads/'.$attachments->images) }}" alt="img"
...
<script srcs="{{URL::asset('assets/plugins/telephoneinput/inttelephoneinput.js')}}"></script>
@foreach($attachment as $attachment)
<script type='text/javascript'>
$(document).ready(function() {
    $('#delete-{{ $attachment->id }}').click(function(event) {
        ...
        $.ajax({
            ...
            success: function(d, b) {
                console.log(b);
                $('#delete-{{ $attachment->id }}').removeClass('d-inline').addClass('d-none');
            }.bind(this)
        });
